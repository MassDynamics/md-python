...
_WORKFLOW_GUIDE = {
    ...
    "tool_index": {
        ...
        "dataset_tools": {
            ...
            "wait_for_dataset": "Poll a pipeline dataset until terminal state. Optional: poll_seconds (default 5), timeout_seconds (default 45).",
        ...
        "pipeline_tools": {
            ...
            "run_dose_response": "Fit 4-parameter log-logistic dose-response curves. Prefer run_dose_response_from_upload (auto-resolves dataset ID) or run_dose_response_bulk (many jobs).",
            "run_dose_response_from_upload": "PREFERRED for single DR job: resolves input_dataset_ids from upload_id automatically. Optional: if_exists='skip' (default) to skip already-submitted jobs; sample_metadata omit to auto-fetch from upload.",
            "run_dose_response_bulk": "PREFERRED for many DR jobs: submits all jobs in one call, caches initial datasets per upload, skips duplicates by default (if_exists='skip'). Each job needs upload_id, dataset_name, sample_names, control_samples.",
        },
        "utility_tools": {
