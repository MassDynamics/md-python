...
_WORKFLOW_GUIDE = {
    ...
    "workflows": {
        "A_upload_new_data": {
            ...
            "steps": [
                ...
                "1b. md_format / md_format_metabolite ONLY: run the ID-SHAPE PREFLIGHT (see constraints) — read the header + first ~5 rows of each DATA file and confirm the primary-ID shape (peptide ModifiedSequence is inline UniMod, protein ProteinGroup is UniProt accessions, SampleName matches experiment_design). STOP and tell the user if the IDs look like a tool's native annotation. For an md_format PTM/phospho experiment, the protein table is REQUIRED and the peptide table is OPTIONAL — upload BOTH together via filenames=[protein_file, peptide_file]; never the peptide table alone.",
        ...
        "D_format_conversion": {
            ...
            "steps": [
                ...
                "PEPTIDE/PTM (phospho) experiments: the md_format PROTEIN table is REQUIRED and the PEPTIDE table is OPTIONAL — upload BOTH together via filenames=[protein_file, peptide_file], never the peptide alone. The peptide table needs a Unique column and a ProteinGroupId that matches the protein file's ProteinGroup->ProteinGroupId mapping (get_md_format_spec(entity_type='peptide')). ModifiedSequence must be INLINE UniMod (PEPT(UniMod:21)IDE; N-term mods before residue 1). Proteome Discoverer exports native '[prev].PEPT.[next] | NxMod [loci]' which must be CONVERTED first (Phospho->21, Oxidation->35, Carbamidomethyl->4, Acetyl->1, TMTpro->2016, Met-loss->765, Met-loss+Acetyl->766; honour the Nx multiplier; stripping every (UniMod:NN) must reproduce StrippedSequence). ~15% of PD rows carry an UNLOCALISED modification ([S/Y], [S/T], bare [S]) with no defined residue — inline UniMod needs a specific site, so ASK the user whether to (a) assign to the first candidate residue of an allowed type or (b) drop the unlocalised rows (optionally (c) assign-to-first plus a sidecar flagging inferred sites). NEVER auto-decide. See the md-mcp-ops skill's pd_to_md_format recipe + pd_to_md_peptide.py for a working converter.",
    ...
    "tool_index": {
        ...
        "dataset_tools": {
            ...
            "query_datasets": "Paginated filter search over datasets (upload_id/state/type/search). 50/page. The server-side type enum accepts ONLY DEMO, DOSE_RESPONSE, DOSE_RESPONSE_AGGREGATE, ENRICHMENT, IMPUTATION, INTENSITY, NORMALISATION_AND_IMPUTATION, PAIRWISE — type=['ANOVA'] is a 400. ANOVA datasets DO exist (type field can be ANOVA); you just cannot filter by it, so use search= or list_datasets(type_filter='ANOVA'). Prefer list_datasets(upload_id=...) for a single upload.",
            "list_dataset_tables": "Discover the CASE-SENSITIVE table names a dataset exposes. Call BEFORE download_dataset_table. Catalogued: INTENSITY (Protein_/Gene_/Metabolite_/Peptide_/PTM_ Intensity+Metadata), PAIRWISE (output_comparisons, runtime_metadata), DOSE_RESPONSE (output_curves, output_volcanoes, input_drc, runtime_metadata), ENRICHMENT/GSEA (output_comparisons = the GSEA RESULTS table, database_metadata, runtime_metadata), ORA (ora_results), ANOVA (anova_results, runtime_metadata). Any other type returns catalogued=false, meaning its table names CANNOT be enumerated — do NOT guess them, ask the user or use the visualisation module.",
            "download_dataset_table": "Get a presigned download URL for a dataset table (csv/parquet). Pass output_path to stream to disk instead. table_name is case-sensitive ('Protein_Intensity', not 'protein_intensity') — get it from list_dataset_tables, never by guessing; a wrong name is always a 404 and retrying with another guess never works. GSEA/ENRICHMENT results download under table_name='output_comparisons' — the SAME name PAIRWISE uses, which is correct and not a mix-up; 'output_gsea'/'output_enrichment'/'output_pathways' do not exist. ANOVA results are 'anova_results'; ORA results are 'ora_results'.",
    ...
    "constraints": [
        ...
        "NEVER read proteomics data files (DIA-NN report.tsv, MaxQuant proteinGroups.txt, Spectronaut exports, md_format tables) — upload them as-is. BOUNDED EXCEPTION for md_format / md_format_metabolite uploads ONLY: before the upload you MUST read the HEADER + first ~5 data rows of each data file and confirm the primary-ID columns have the expected shape — peptide ModifiedSequence is inline UniMod (PEPT(UniMod:21)IDE), NOT a native annotation like Proteome Discoverer's '[K].PEPT.[V] | 1xPhospho [T4]'; protein ProteinGroup is UniProt accession(s); SampleName matches experiment_design. This ID-shape check on a few lines is distinct from ingesting/parsing the full table. If the IDs look like another tool's native output, STOP — name the tool the format resembles and what is wrong — instead of uploading malformed data. read_csv_preview stays within this boundary.",
        "md_format COMPOSITION: a PTM/phospho experiment is ONE md_format upload — the PROTEIN table is REQUIRED, the PEPTIDE (PTM) table is OPTIONAL, and both are uploaded TOGETHER via filenames=[protein_file, peptide_file] with source='md_format'. NEVER upload a peptide table alone: md-converter builds the protein output first and raises FileNotFoundError if it is absent (md_format/runner.py:27,38, reader.py:46-48); the client rejects a peptide-only md_format upload. The peptide table additionally needs a Unique column AND a ProteinGroupId that uses the IDENTICAL ProteinGroup->ProteinGroupId mapping as the protein file (see get_md_format_spec('peptide')).",
