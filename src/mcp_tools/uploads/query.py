...
def query_uploads(
    ...
    Args:
        ...
        source: filter by acquisition software / format used to produce the
            files. Server-accepted values come from
            workflow/app/models/experiment.rb:27-35 VALID_SOURCE_FORMATS:
            ``diann_tabular``, ``tims_diann``, ``spectronaut``, ``maxquant``,
            ``md_format``, ``md_format_gene``, ``unknown``. The MCP create
            tools only allow the first six (``unknown`` is rejected client-
            side by md_python.resources.v2.uploads.ALLOWED_UPLOAD_SOURCES);
            but legacy uploads may exist on the server with
            ``source='unknown'`` so this filter accepts it for query.
            Examples: ["diann_tabular"], ["maxquant", "spectronaut"].
